...
The primal dual solver has way more parameters that have to be specified by the user: actually, there is $\lambda$, $\theta$, $\simga$, $\thau$ and in the gradient descent solver we only have $\lambda$ and $\alpha$. It is harder to find appropriate parameters for the primal-dual solver. Furthermore, the primal-dual solver is very sensitive to changing parameter setting and may result in diverging results easily when changing lambda. It also takes a lot of time to find good parameter setting for the primal dual solver compared to the gradient descent solver. 
...
Lest a little comparison of the results produced by both solvers using their optimal values and parameter settings. The results are shown in figure $FIGURE$. This comparison directly confirms our conclusion.
