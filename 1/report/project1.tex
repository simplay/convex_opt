...
To minimize our cost function $E$ we rely on the \emph{gradient descent} approach. This is an iterative method that can be used to solve energy minimiazion problems, such as our cost function. In detail, the following steps have to be performed when implementing the gradient descent approach:
\begin{enumerate}
...
% Link explicitely this described method to our problem statement!

\begin{align}
	u_{c}(i,j) = u_{c}(i,j) - \alpha \frac{\partial{E}}{\partial{u_{c} (i,j)}}(i,j)
\end{align}

where $c$ denotes the substitute for either one of the following r,g,b color channels. 
