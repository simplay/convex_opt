...
where C denotes the dhree diffeent color channels. $\Omega_{C}$ is defined such that $\Omega_{C}(x,y) = 1$ if the pixel value at $(x,y)$ is \emph{valid}$\footnote{this means that the pixel at location (x,y) is valid for the bayer color mask C}$ and $\Omega_{C}(x,y) = 0$ when the data is missing.

MORE GLUE

According to these definitions we see that we can instead minimize three independent convex problems:
...
where $c$ denotes the substitute for either one of the following r,g,b color channels. $\Omega_{}$  
